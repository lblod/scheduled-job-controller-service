...
export class ScheduledJob {
  ...
  async run() {
    try {
      console.log(`ScheduledJob: Executing ${this.uri} [${this.frequency}]`);
      await runScheduledJob(this.uri);
    } catch (e) {
      console.error(`ScheduledJob: Something unexpected went wrong while executing ${this.uri}`);
      console.error(e);
      const metadata = await getScheduledJobData({ uri: this.uri }).catch((e) => {
        console.log('failed to get metadata for job', e);
        return null;
      });
...
export async function runScheduledJob(uri, maxConcurrentJobEnabled = true) {
  ...
  if (!metadata) {
    console.log(`ScheduledJob: Job not created: No metadata for ${uri} could be found.`);
    return false;
  }
  if (maxConcurrentJobEnabled && MAX_CONCURRENT_JOBS > 0) {
    ...
    if (busyJobCount >= MAX_CONCURRENT_JOBS) {
      console.log(`ScheduledJob: Job creation skipped for ${uri}: ${busyJobCount} busy jobs >= ${MAX_CONCURRENT_JOBS} max concurrent jobs`);
