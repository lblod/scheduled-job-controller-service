...
export class ScheduledJobsManager {
  ...
  /**
   * Returns if the scheduled-job exists
   *
   * @param job
   * @returns {*}
   */
  has(job) {
    ...
    return this.inMemoryJobsByURI.has(job);
  }

  /**
   * Returns the scheduled-job if it exists, undefined otherwise
   *
   * @param job
   * @returns {*}
   */
  get(job) {
    ...
    return this.inMemoryJobsByURI.get(job);
  }
   ...
   * @returns {ScheduledJob}
   */
  add(job, frequency =  undefined) {
    if (job) {
      if (typeof job === 'string' || job instanceof String) {
        if(!frequency)
          throw "no frequency provided, can not creat job"
